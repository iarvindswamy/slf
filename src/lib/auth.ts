...
/**
 * Used by API routes for role checks.
 * Accepts optional request:
 * - with request → Authorization Bearer token
 * - without request / no bearer → Firebase session cookie
 */
export async function getCurrentUser(
  request?: Request,
): Promise<PermissionUser | null> {
  try {
    ...
    // 1) Bearer token when request is provided
    if (request) {
      authUser = await getOptionalAuth(request);
    }
